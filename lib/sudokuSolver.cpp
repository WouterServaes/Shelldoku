...
bool SudokuSolver::CanBePlaced(const Solver& solver, ValueLocation location, SudokuValue value)
{
  InitiateSolver(solver.solvertType);
...
bool SudokuSolver_bitmasks::CanBePlaced(const Solver& solver, ValueLocation location, SudokuValue value) const noexcept
{
  if(!value.has_value()
  || !XYToSquareIndex(solver.size,solver.sectionSize, location).has_value()
  || !GeneratedBitMasks(solver)
  || location.first >= solver.size
  ...
  return !((bitMasksColumns[location.first] >> value.value()) & 1)
      ...
      && !((bitMasksSquares[XYToSquareIndex(solver.size, solver.sectionSize, location).value()] >> value.value()) & 1);
}
...
void SudokuSolver_bitmasks::SetBitMasks(const Solver& solver, ValueLocation location, SudokuValue value) noexcept
{
  if(!value.has_value()
  || !XYToSquareIndex(solver.size, solver.sectionSize, location).has_value()
  || !GeneratedBitMasks(solver)
  ...
  bitMasksSquares[XYToSquareIndex(solver.size, solver.sectionSize, location).value()] |= 1 << v;
}

void SudokuSolver_bitmasks::ResetBitMasks(const Solver& solver,ValueLocation location, SudokuValue value) noexcept
{
  ...
  bitMasksSquares[XYToSquareIndex(solver.size, solver.sectionSize, location).value()] &= ~(1 << value.value());
}

bool SudokuSolver_bitmasks::GeneratedBitMasks(const Solver& solver) const noexcept
{
  return (!(bitMasksRows.empty() || bitMasksColumns.empty() || bitMasksSquares.empty()))
   && bitMasksRows.size() == solver.size && bitMasksColumns.size() == solver.size && bitMasksSquares.size() == solver.size;
}

bool SudokuSolver_bitmasks::Solve(Solver& solver, ValueLocation location)
{
  if(location.first == solver.size - 1 
  && location.second == solver.size) {
    return true;
  }
