...
class SudokuSolver_
{
  public:
    ...
    [[nodiscard]] virtual bool CanBePlaced(const Solver& solver, ValueLocation location, SudokuValue value) const noexcept = 0;
...
class SudokuSolver_bitmasks final: public SudokuSolver_
{
  public:
    ...
    [[nodiscard]] bool CanBePlaced(const Solver& solver, ValueLocation location, SudokuValue value) const noexcept override;
  ...
  private:
    ...
    void SetBitMasks(const Solver& solver, ValueLocation location, SudokuValue value) noexcept;
    // Helper to reset all 3 bitmasks for location/value
    void ResetBitMasks(const Solver& solver, ValueLocation location, SudokuValue value) noexcept;
